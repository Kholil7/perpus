const scriptURL = 'https://script.google.com/macros/s/AKfycbwDHCA8N3saB4mMVX7vZGLPoXp9W-KNSK0XdESLqGTdcxFzldUmD93v-NpPsdBFfnNujQ/exec';
const form = document.forms['data-pinjam-siswa'];
const content = document.querySelector('.content');
const pinjem = document.querySelector('.pinjamSiswa');
...
form.addEventListener('submit', e => {
  ...
  fetch(scriptURL, { method: 'POST', body: new FormData(form)})
    .then(response => {
      if (response.ok) {
        alert('Sis & Bro, Selamat data sudah berhasil dikirim😎🚀.');
        form.reset();
        ...
        content.innerHTML = '<p>"Semangat Membaca Buku 📚<br>Tetap Semangat Untuk Masa Depan Yang Cerah 🔑 "<br><a href = "../index.html">Kembali Ke Home</a></p>';
      } else {
...
function tambahkanTandaPetikDepan() {
  ...
  document.getElementById("nisn").value = nilaiDenganTandaPetikDepan;
}
